<div align="center">

```
███╗   ██╗██╗██████╗ ██╗   ██╗███╗   ██╗
████╗  ██║██║██╔══██╗██║   ██║████╗  ██║
██╔██╗ ██║██║██████╔╝██║   ██║██╔██╗ ██║
██║╚██╗██║██║██╔═══╝ ██║   ██║██║╚██╗██║
██║ ╚████║██║██║     ╚██████╔╝██║ ╚████║
╚═╝  ╚═══╝╚═╝╚═╝      ╚═════╝ ╚═╝  ╚═══╝
```

### Full-Stack Engineer · Creative Technologist · Builder of Things

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=15&duration=3000&pause=1000&color=00FFB3&center=true&vCenter=true&width=650&lines=Full-Stack+Engineer+%7C+Delhi%2C+India+%F0%9F%87%AE%F0%9F%87%B3;3D+Web+Experiences+with+Three.js+%26+React;AI+Agents+%7C+Flutter+%7C+Cloud+%7C+Game+Dev;Always+learning%2C+always+shipping+%E2%9A%A1)](https://git.io/typing-svg)

</div>

---

## 👾 About Me

```yaml
name:      Nipun Kachwaha
alias:     thakurxnipun / desireofrana
role:      Full-Stack Engineer & Creative Technologist
location:  Delhi, India · UTC+05:30
portfolio: https://space3dportfolio.netlify.app

currently_working:
  - AI-Powered Wholesale Order Aggregation System
  - AI-Powered Driver Distraction Detection System

interests:
  - Generative AI in 3D Web Experiences
  - AI-Driven Decentralized Apps
  - Machine Learning for Game Development
  - Algorithmic UI/UX Design

fun_fact: "I build Projects in space because the tech needs more gravity 🚀"
```

---

## 🛠️ Tech Stack

<div align="center">
  
<img src="https://skillicons.dev/icons?i=js,ts,python,java,cpp,cs,c,rust,php,linux,wordpress,react,threejs,nextjs,tailwind,figma,vite,ps,kubernetes,net,nodejs,flutter,express,nestjs,fastapi,laravel,mongodb,postgres,mysql,redis,firebase,supabase,gcp,aws,azure,docker,cloudflare,git,tensorflow,pytorch,opencv,sklearn,gitlab,spring,django" />

---

</div>

## 🌐 Connect With Me

<div align="center">

[<img src="https://cdn.jsdelivr.net/gh/twitter/twemoji@14.0.2/assets/svg/1f310.svg" width="48" height="48"/>](https://space3dportfolio.netlify.app)
[![LinkedIn](https://skillicons.dev/icons?i=linkedin)](https://www.linkedin.com/in/nipun-/)
[![GitHub](https://skillicons.dev/icons?i=github)](https://github.com/NipunKachwaha)
[![Instagram](https://skillicons.dev/icons?i=instagram)](https://www.instagram.com/thakurxnipun/)
[![X](https://skillicons.dev/icons?i=twitter)](https://x.com/desireofrana)

</div>

---

<div align="center">

**Thanks for visiting · Check out my Projects ✦**

</div>
